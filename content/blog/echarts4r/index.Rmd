...
I used Facebook's prophet package (see <https://facebook.github.io/prophet/docs/quick_start.html#r-api>) to forecast the sum of daily departure delays for JFK airport. The prophet package already comes with a built-in visualization function using dygraphs, but this article is about echarts.
...
While the forecast is not particularly useful, the tooltip and zoom give the viewer more precision than a static display.
...
For this article, I demonstrate chart linking with a visualization of time series decomposition.

### Time series decomposition

```{r decomp}
decomp <- predict(model) %>% # analyze history
  mutate(
    y = history$y, 
    remainder = y - trend - weekly - additive_terms
  )
base_chart <- decomp %>% 
  e_charts(x = ds, height = 200) %>%
  e_tooltip() %>% 
...
y <- base_chart %>% 
  e_line(y, symbol = "none", legend = FALSE) %>% 
...
weekly <- base_chart %>% 
  e_line(weekly, symbol = "none", legend = FALSE) %>% 
  e_datazoom(show = FALSE, toolbox = FALSE) %>% 
...
e_arrange(y, trend, weekly, remainder, nrow = 4)
```
