...
# TODO: Fix output format (model not predicting 0 or 1)
# TODO: Consider other LoRA configurations
# TODO: Use Llama 3 instruct chat template https://pytorch.org/torchtune/stable/tutorials/chat.html
...
| 3: Llama 3.2-3B LoRA fine-tuned| 0.98 | 0.99 | 0.97 | 1 |

TODO: Update with actual results
TODO: Read about fine-tuning Llama for classification specifically
TODO: Find out if it's possible to get probabilities for classes
...
TODO: Analyze examples per second. vLLM is highly optimized for inference, bringing the speed for the larger Llama model above that of ModernBERT.
TODO: Analyze ease of training
TODO: Analyze ease of deployment for inference

Here, I set up training and inference using Modal and open source libraries. This process can be shortened by using a managed service like [Fireworks](https://fireworks.ai), which provides an inference and training API. With torchtune, LoRA and the increasing abundance of GPU servers, fine-tuning LLMs isn't significantly more expensive or difficult than training ModernBERT. Both require training data. If you do not have training data, the few-shot learning approach is a good alternative. It can also be used to bootstrap a training dataset, especially when combined with a full size LLM like Llama-3.1-405B or gpt-4o.

Effort comparison, assuming complete greenfield project:

- Using LLMs via API is easy. If prompting is enough for your use case and the amount of requests isn't enormous, it's a good solution.
- Running ModernBERT is medium. Runs on any GPU and also on CPU (slowly). Can start on a laptop. Scales to extremely high request volumes.
- Running LLMs with LoRA is relatively hard compared to the other two. Requires more configuration of training (data format, conversation template, libraries like flash attention) and inference (loading LoRA adapter, generation arguments). Use this if the NLP task is complex and you have training data.
