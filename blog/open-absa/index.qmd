---
title: "Open fine-tuned LLMs for aspect-based sentiment analysis"
...
I'm happy to report that a LoRA fine-tune of Llama-3 8B achieves similar performance (82.1 F1, vs. 83.8 for GPT-3.5). The model is small enough to run on a single GPU and has open weights.
...
I used [axolotl](https://github.com/OpenAccess-AI-Collective/axolotl) to train the model. Training ran on [Modal](https://modal.com) using 2x A100 GPUs for the 7 and 8 models and 2x H100 for the 70B model. Inference ran on a single H100 for all models.
...
I've added `[INST]` and `[/INST]` tokens during training to indicate the start and end of an instruction. When I tried the [input_output format](https://axolotl-ai-cloud.github.io/axolotl/docs/input_output.html#you-may-not-want-prompt-templates), so a template-free approach, the model generated malformed outputs that weren't valid JSON.
...
During inference using vLLM I set these [sampling parameters](https://docs.vllm.ai/en/latest/offline_inference/sampling_params.html):
...
The repetition penalty prevents the model from repeating the same tokens over and over. A low temperature and strict settings for top_p and top_k encourage following the JSON format of the expected output, e.g.
...
Pretty printed here, the actual output is a single line of text. 256 tokens is enough to cover all aspects of the longest review in the dataset.
...
| Model | F1 ↓ | Parameters |
|-------|----| -----------|
| GPT-3.5 fine-tuned | 83.76 | 175B |
| Llama-3 70B (4bit) | 83.40 | 70B |
| Llama-3 8B | 82.13 | 8B |
| Mistral-7B | 81.97 | 7B |
| InstructABSA | 79.30 | 0.2B |

GPT-3.5 fine-tuned remains the most performant model. Llama-3 8B and Llama-3 70B are less than 1 F1 percentage point behind. Scaling from 8B to 70B only yields a 1.27 F1 percentage point improvement. This gain may be larger when a higher LoRA rank is used or when less intense quantization is applied.
